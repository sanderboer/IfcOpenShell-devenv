...
;; (set-face-attribute 'show-paren-match nil :inherit 'default :background "grey10" :foreground "gray" :inverse-video nil :weight 'normal)
...
(defun sndr-cpp-rtags ()
  (interactive)
  ;; (use-package rtags :ensure t
  ;;              :config(progn
  ;;                       (setq rtags-install-path (expand-file-name "~/.emacs.d/rtags" ))  
  ;;                       (add-to-list 'load-path  (concat rtags-install-path "/rtags-" rtags-package-version "/src"))
  ;;                       (setq rtags-path (concat rtags-install-path "/rtags-" rtags-package-version "/bin"))
  ...
  ;;             )
  (setq rtags-install-path (expand-file-name "~/.emacs.d/rtags"))
  ;; (setq rtags-package-version "2.31")
  ;; (add-to-list 'load-path  (concat rtags-install-path "/rtags-" rtags-package-version "/src"))
  (add-to-list 'load-path
               (concat rtags-install-path "/rtags-git/share/emacs/site-lisp/rtags"))
  (require 'rtags)
  ;; (setq rtags-path (concat rtags-install-path "/rtags-" rtags-package-version "/bin"))
  (setq rtags-path (concat rtags-install-path "/rtags-git/bin"))
  (rtags-start-process-unless-running)
  (rtags-enable-standard-keybindings)
  (add-hook 'kill-emacs-hook 'rtags-quit-rdm)
  ...
  ;; (use-package company-rtags :ensure t :config(progn
  ...
  (define-key c-mode-base-map (kbd "M-t s") 'rtags-find-symbol-at-point)
  (define-key c-mode-base-map (kbd "M-t f") 'rtags-find-functions-called-by-this-function)
  (define-key c-mode-base-map (kbd "M-t F") 'rtags-find-all-references-at-point)
  (define-key c-mode-base-map (kbd "M-[") 'rtags-location-stack-back)
  (define-key c-mode-base-map (kbd "M-]") 'rtags-location-stack-forward)
  (define-key c-mode-base-map (kbd "M-\\") 'rtags-location-stack-visualize))
